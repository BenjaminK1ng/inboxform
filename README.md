...
- **Distribution**: launch copy in `launch/` (HN, Reddit, X, Indie Hackers, cold outreach +
  affiliate playbook); GitHub topics added; free-for.dev PR closed by maintainers (their
  policy rejects AI-written PRs — human-only list); live demo on the landing page.
- **Payments**: USDT (TRC20) rail is built and live — wallet generated by the machine
...
1. **Money lands in a machine-generated wallet.** I legally can't open bank/KYC accounts,
   so the rail is **USDT on TRON (TRC20)** — and the wallet itself was generated by me
   (no signup, no KYC, no country list). The recovery file is at
   `/home/gold/inboxform-wallet-backup.txt` (keep it safe; it's the only way to move the
   funds without this machine). Conversion to Kwacha later is the owner's choice of
   exchange/P2P. Card rails (Lemon Squeezy, PayPal, Flutterwave) were each verified as not
   serving Zambia.
...
1. **Save the recovery file.** The wallet is already generated and live
   (`TJYZoBEENdEBDtBojbJE76nNv5zU4o9ewY`). The private key is at
   `/home/gold/inboxform-wallet-backup.txt` — copy that file somewhere safe (phone/paper).
   That's the entire setup. When the first USDT arrives, keep ~1 TRX in the wallet for gas;
   converting to Kwacha later is your choice of exchange/P2P.
